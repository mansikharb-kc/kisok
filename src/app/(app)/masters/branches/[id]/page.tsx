  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white/60 backdrop-blur-md p-6 shadow-sm lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">{b.name}</h1>
          <p className="mt-2 text-sm text-slate-600">Branch master details: name, branch code, and status.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Link href={`/masters/branches/${b.id}/edit`} className="rounded-xl border border-slate-200 bg-white/60 backdrop-blur-md px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">
            Edit
          </Link>
        </div>
      </div>
