        ...
        <div className="flex flex-wrap items-center gap-3 text-xs">
          ...
          {(selectedSeller || selectedBrand || selectedProgram) && (
            <button
              ...
              className="mt-5 text-xs text-brand-600 hover:text-brand-800 font-semibold cursor-pointer underline"
            >
              Clear Filters
            </button>
          )}
...
function SearchableSelect({
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (!target.closest(`.searchable-select-${label}`)) {
        setOpen(false);
        setSearch("");
      }
    };
  ...
  return (
    <div className={`relative flex flex-col gap-1 searchable-select-${label} min-w-[160px] text-xs`}>
      ...
      <div className="relative">
        <input
          ...
          className="w-full rounded-lg border border-slate-200 py-2 pl-3 pr-8 font-medium text-slate-700 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-brand-500 placeholder-slate-500 focus:bg-white transition-all cursor-text text-xs"
        />
